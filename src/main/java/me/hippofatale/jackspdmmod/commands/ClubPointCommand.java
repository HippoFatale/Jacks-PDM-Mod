...
public class ClubPointCommand {
    public ClubPointCommand(CommandDispatcher<CommandSource> dispatcher) {
        dispatcher.register(Commands.literal("길드자금")
    ...
    private int withdrawClubPoints(CommandSource source, int amount) throws CommandSyntaxException {
        try {
            ...
            if (!ClubManager.belongingClubs.containsKey(player.getUUID())) {
                ...
                return 0;
            } else if (!ClubManager.belongingClubs.get(player.getUUID()).isPresident(player.getUUID())) {
                player.displayClientMessage(new TranslationTextComponent("message.jackspdmmod.not_president"), false);
                return 0;
            } else if (ClubManager.belongingClubs.get(player.getUUID()).getClubPoints() < amountHundreds * 100) {
                ...
                ClubManager.belongingClubs.get(player.getUUID()).takeClubPoints(amountHundreds * 100);
