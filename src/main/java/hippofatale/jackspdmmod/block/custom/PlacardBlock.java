...
import hippofatale.jackspdmmod.networking.packet.SetPlacardScreenS2CPacket;
...
public class PlacardBlock extends StandingSignBlock {
    ...
    public PlacardBlock(Properties properties, HomeType homeType, HomeSize homeSize, BigDecimal price, WoodType woodType) {
    ...
    public ActionResultType use(BlockState blockState, World world, BlockPos blockPos, PlayerEntity player, Hand hand, BlockRayTraceResult rayTraceResult) {
        if (!world.isClientSide()) {
            ...
            if (tileEntity instanceof PlacardTile) {
                ...
                placard.setPlotType(plotType);
                placard.setPlotSize(plotSize);
                placard.setPlotPrice(plotPrice);
